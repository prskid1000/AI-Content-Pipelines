...
MODEL_CHARACTER_META_SUMMARY = "qwen3.5-35b-a3b"  # Model for meta-summary generation
...
#   True  -> ask LM Studio to cast voices by personality/role
#   False -> fall back to male_/female_ prefix heuristic
AUTO_ASSIGN_VOICE = True

# Where to write the character -> voice-stem mapping consumed by 2.story.py
SPEAKER_MAP_OUTPUT = "../input/1.story.speakers.txt"
...
class CharacterManager:
    ...
    def assign_voices_to_characters(self, characters, story_text: str = ""):
        """Assign voices to characters.

        If AUTO_ASSIGN_VOICE is True, ask LM Studio to cast voices by
        personality/role. Falls back to the prefix heuristic on any LLM
        failure so the pipeline never stalls.
        """
        ...
        if AUTO_ASSIGN_VOICE:
            print("\nAUTO_ASSIGN_VOICE=True -> asking LM Studio to cast voices...")
            try:
                llm_mapping = self._llm_assign_voices(characters, story_text)
                updated_character_voices.update(llm_mapping)
                self.character_voices.update(updated_character_voices)
                print("\n=== FINAL CHARACTER-VOICE MAPPING (LLM) ===")
                for char in characters:
                    print(f"- {char}: {updated_character_voices.get(char, 'UNASSIGNED')}")
                self.update_character_alias_map_file(updated_character_voices)
                return updated_character_voices
            except Exception as e:
                print(f"LLM casting failed ({e}); falling back to prefix heuristic.")
    ...
    def update_character_alias_map_file(self, character_voices_dict):
        """Write character -> voice-stem mapping to input/1.story.speakers.txt.

        Format (consumed by 2.story.py):
            # comment
            <char_tag>  <voice_stem>
        """
        out_path = SPEAKER_MAP_OUTPUT
        try:
            ...
            if os.path.exists(out_path):
                ...
                print(f"Created backup: {out_path}.backup")

            # Compute column width so the file looks aligned when hand-edited
            max_char = max((len(c) for c in character_voices_dict), default=20)
            col_width = max(max_char + 2, 22)

            with open(out_path, "w", encoding="utf-8") as f:
                f.write("# Character tag -> voice stem mapping\n")
                f.write("# Generated by 1.character.py\n")
                f.write("# Format: <character_tag>  <voice_stem>\n")
                f.write("# Voice stems are resolved by globbing voices/**/<stem>.wav\n")
                f.write("# Each stem must have a sibling <stem>.reference.txt\n\n")
                for character, voice in character_voices_dict.items():
                    f.write(f"{character.ljust(col_width)}{voice}\n")

            print(f"Wrote speaker map: {out_path}")
            print("Format: <character_tag>  <voice_stem>")
            
        except Exception as e:
            print(f"Error writing speaker map file: {e}")
    ...
    # ------------------------------------------------------------------
    # LLM-powered voice casting (triggered when AUTO_ASSIGN_VOICE=True)
    # ------------------------------------------------------------------

    def _llm_assign_voices(self, characters: list, story_text: str) -> dict:
        """Ask LM Studio to cast voices to characters by personality/role.

        Returns {char_tag: voice_stem}. Raises on failure — caller decides
        whether to fall back to the prefix heuristic.

        Rules enforced via strict JSON schema:
          - every character is mapped
          - male_* characters get a male voice stem
          - female_* characters get a female voice stem
          - voices should not repeat unless the pool is exhausted
        """
        if not characters:
            return {}
        if not self.male_voices and not self.female_voices:
            raise RuntimeError(
                "No voice stems available for LLM casting "
                f"(language={self.language}, region={self.region})"
            )

        # Brief context for the model — first ~4000 chars is plenty for casting
        story_excerpt = (story_text or "")[:4000]

        system_prompt = (
            "You are a casting director assigning voice actors to characters in an audio drama.\n"
            "Given: a list of character tags, a story excerpt, and pools of available "
            "male and female voice stems, pick the best voice stem for each character "
            "based on their role, personality, and context.\n"
            "\n"
            "Hard rules:\n"
            "- Every character tag in the input must appear in the output.\n"
            "- If a character tag starts with 'male_', pick ONLY from the male voice pool.\n"
            "- If a character tag starts with 'female_', pick ONLY from the female voice pool.\n"
            "- If neither prefix, infer gender from context and pick accordingly.\n"
            "- Do not invent voice stems; use the exact strings from the pools.\n"
            "- Avoid reusing the same voice stem for two characters unless the "
            "  same-gender pool is smaller than the number of same-gender characters.\n"
        )

        user_prompt = (
            f"Characters:\n- " + "\n- ".join(characters) + "\n\n"
            f"Available male voice stems: {', '.join(self.male_voices) or '(none)'}\n"
            f"Available female voice stems: {', '.join(self.female_voices) or '(none)'}\n\n"
            f"Story excerpt (for personality context):\n{story_excerpt}\n"
        )

        # Strict JSON schema: object whose keys are exactly the character tags
        properties = {
            char: {"type": "string", "enum": self.male_voices + self.female_voices}
            for char in characters
        }
        response_format = {
            "type": "json_schema",
            "json_schema": {
                ...
                "schema": {
                    ...
                    "required": list(characters),
                },
                "strict": True,
            },
        }

        # Direct LM Studio call (bypasses the flag-based wrapper — cleaner for this)
        payload = {
        ...
        print(f"Asking LM Studio to cast {len(characters)} characters...")
        resp = requests.post(
        ...
        content = _strip_think_tags(resp.json()["choices"][0]["message"]["content"])
        try:
            mapping = json.loads(content)
        except json.JSONDecodeError as e:
            raise RuntimeError(f"LM Studio returned non-JSON content: {content[:200]}") from e

        # Validate and repair: enforce gender rules and no-repeat-unless-forced
        male_set = set(self.male_voices)
        female_set = set(self.female_voices)
        used: set = set()
        repaired: dict = {}
        for char in characters:
            picked = mapping.get(char)
            if not isinstance(picked, str):
                raise RuntimeError(f"LLM omitted or gave non-string voice for '{char}'")
            is_male = char.lower().startswith("male_")
            is_female = char.lower().startswith("female_")
            pool = self.male_voices if is_male else self.female_voices if is_female else (self.male_voices + self.female_voices)
            if is_male and picked not in male_set:
                raise RuntimeError(f"LLM gave non-male voice '{picked}' for male character '{char}'")
            if is_female and picked not in female_set:
                raise RuntimeError(f"LLM gave non-female voice '{picked}' for female character '{char}'")
            if picked in used and len([c for c in characters if c.lower().startswith("male_" if is_male else "female_")]) <= len(pool):
                # Pool had room — LLM reused unnecessarily. Pick first unused in pool.
                alt = next((v for v in pool if v not in used), None)
                if alt:
                    print(f"  [repair] '{char}': LLM picked reused '{picked}', switching to '{alt}'")
                    picked = alt
            used.add(picked)
            repaired[char] = picked
            print(f"  {char} -> {picked}")
        return repaired
    ...
    def preprocess_story(self, story_text, resumable_state: ResumableState | None = None):
        ...
        if resumable_state and resumable_state.is_character_voices_complete():
            ...
            if cached_voices:
                ...
                # Assign voices to characters
                character_assignments = self.assign_voices_to_characters(characters, story_text)
                # Save to checkpoint if resumable mode enabled
                if resumable_state:
