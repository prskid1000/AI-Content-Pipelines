...
class LocationGenerator:
    ...
    def _update_workflow_prompt(self, workflow: dict, location_name: str, description: str) -> dict:
        """Update the workflow with location-specific prompt - OPTIMIZED FOR NEWS CHANNEL."""
        prompt = f"Create a 16K ultra-high-resolution, detailed location environment illustration in the style of {ART_STYLE} optimized for news and information channel content. The scene should be atmospheric, immersive, and precisely matching the location description with fine-level detailing, rich color palette, proper lighting, depth of field, and environmental storytelling appropriate for news broadcasts. For news-related locations, emphasize professional settings such as modern newsrooms, broadcast studios, office environments, or relevant news locations with broadcast-quality aesthetics, professional lighting, and credible journalistic environments. No characters should be visible in this location scene.\n\n Location Description = {description}. Strictly, Accurately, Precisely, always must Follow {ART_STYLE} Style for environmental art. All Colourings, Styles, Shapes, Textures, Lightings and Detailing, must be **exactly same/ identical/as it is** in the scene text-description. When appropriate for news content, incorporate elements typical of news environments: modern furniture, professional lighting fixtures, newsroom equipment, studio settings, or relevant news location details." +  "\n __NOTE__:" + " ".join(EXTRA_PROMPT_PHRASES)
