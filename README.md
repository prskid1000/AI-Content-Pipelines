...
│   │   ├── story.json          # Multi-speaker TTS template (OmniVoice); LoadAudio nodes added per chunk by 2.story.py
...
|---|--------|---------|-------------|--------------|--------------|-----------|
| 1 | `gen.audio/scripts/1.character.py` | Character voice assignment & analysis (LLM casting when `AUTO_ASSIGN_VOICE=True`, else prefix heuristic) | `gen.audio/input/1.story.txt`, `gen.audio/voices/` | `gen.audio/input/1.story.speakers.txt` | **LM Studio** | ✅ |
| 2 | `gen.audio/scripts/2.story.py` | Generate main story audio (OmniVoice multi-speaker TTS, up to 10 speakers per chunk) | `gen.audio/input/1.story.txt`, `gen.audio/input/1.story.speakers.txt` | `gen.audio/output/story.wav` | **ComfyUI** | ✅ |
...
├── input/                      # Input files
│   ├── 1.story.txt            # Source story text
│   ├── 1.story.speakers.txt   # Character -> voice-stem map (generated by 1.character.py)
...
# Voice casting mode
AUTO_ASSIGN_VOICE = True  # True = LLM casts voices by personality/role (uses LM Studio with strict JSON schema)
                          # False = fall back to male_/female_ prefix heuristic (no LLM)
SPEAKER_MAP_OUTPUT = "../input/1.story.speakers.txt"  # Output path consumed by 2.story.py
```

Output: `gen.audio/input/1.story.speakers.txt` — space-separated `<character_tag>  <voice_stem>` map. If LM Studio fails, the script silently falls back to the prefix heuristic so the pipeline never stalls.

##### `2.story.py` - Story Audio Generation (OmniVoice multi-speaker)
```python
CHUNK_SIZE = 15  # Number of dialogues/lines per chunk (default, can be overridden with --chunk-size)
MAX_SPEAKERS_PER_CHUNK = 10  # Hard cap from OmniVoice (MAX_SPEAKERS in ComfyUI-OmniVoice-TTS)
ENABLE_RESUMABLE_MODE = True
CLEANUP_TRACKING_FILES = False
...
# Voice resolution
SPEAKER_MAP_FILE = "../input/1.story.speakers.txt"  # Character -> voice-stem mapping
VOICES_ROOT = "../voices"                            # Voice library root (globbed by stem)
COMFYUI_INPUT_DIR = "../../ComfyUI/input"            # Where reference WAVs are staged
STAGED_VOICE_PREFIX = "gen.audio."                   # Filename prefix for staged WAVs
...
1. Parses active characters in the chunk (first-appearance order) and errors if >10 unique speakers.
2. Rewrites `[char_tag] text` lines into `[Speaker_N]: text` markers that OmniVoice understands.
3. Stages each referenced voice WAV into `ComfyUI/input/` as `gen.audio.<stem>.wav` (skipped if already copied).
4. Builds the workflow from the `workflow/story.json` template, injecting one `LoadAudio` node per speaker plus `num_speakers.speaker_N_{audio,ref_text,instruct}` dot-keys (V3 DynamicCombo API format).
5. Reference transcripts are read from the sibling `<stem>.reference.txt`, so Whisper auto-transcription is skipped.
...
graph TD
    ...
    B --> C[gen.audio/input/1.story.speakers.txt]
    B --> D[gen.audio/voices/]
    B -.-> E[LM Studio]
...
| Speech | `gen.audio/scripts/2.story.py` | Story audio generation (OmniVoice multi-speaker) | `gen.audio/input/1.story.txt`, `gen.audio/input/1.story.speakers.txt` | `gen.audio/output/story.wav` | **ComfyUI** | ✅ |
...
Story Text → Scene Images → Motion Descriptions → AV Video Generation → Final Video
     ↓            ↓              ↓                      ↓                  ↓
gen.audio/input/1.story.txt → gen.image/output/scene/*.png → gen.av/input/2.motion.txt → gen.av/output/scene_*.mp4 (with audio) → gen.av/output/final.mp4
```
...
| Category | Script | Purpose | Input Files | Output Files | Dependencies | Resumable |
|----------|--------|---------|-------------|--------------|--------------|-----------|
| Story | `gen.av/scripts/1.story.py` | Parse story for image pipeline | `gen.audio/input/1.story.txt` | `gen.image/input/1.story.txt` | None | ❌ |
| Story | `gen.image/scripts/1.story.py` | Parse story for image pipeline | `gen.audio/input/1.story.txt` | `gen.image/input/1.story.txt` | None | ❌ |
| Story | `gen.audio/scripts/1.character.py` | Character generation | Character inputs | Character outputs | **LM Studio** | ✅ |
| Images | `gen.image/scripts/2.character.py` | Character image generation | Character inputs | `gen.image/output/characters/*.png` | **ComfyUI** | ❌ |
| Images | `gen.image/scripts/2.location.py` | Location image generation | Location inputs | `gen.image/output/locations/*.png` | **ComfyUI** | ❌ |
| Images | `gen.image/scripts/3.scene.py` | Scene image generation | Scene inputs | `gen.image/output/scene/*.png` | **ComfyUI** | ❌ |
| Video | `gen.video/scripts/2.motion.py` (shared) | Generate master prompts for video generation | `gen.audio/input/1.story.txt`, `gen.image/output/scene/*.png` (optional), `gen.image/input/2.character.txt` (optional), `gen.image/input/2.location.txt` (optional) | `gen.av/input/2.motion.txt` | **LM Studio** | ✅ |
| Video | `gen.av/scripts/2.av.py` | Generate AV videos with built-in audio | `gen.audio/input/1.story.txt`, `gen.image/output/scene/*.png`, `gen.av/input/2.motion.txt` (master prompts, optional), `gen.audio/output/story/*.wav` (optional, when SOUND_MODE=AUDIO) | `gen.av/output/scene_*.mp4`, `gen.av/output/frames/*.png` | **ComfyUI** | ✅ |
| Thumbnail | `gen.audio/scripts/9.media.py` | Media processing | Media inputs | Media outputs | **ComfyUI** | ❌ |
| Thumbnail | `gen.audio/scripts/10.thumbnail.py` | Thumbnail generation | Thumbnail inputs | Thumbnail outputs | **ComfyUI** | ❌ |
...
├── gen.audio/input/1.story.txt          # Source story text (dialogue format: [speaker] "dialogue")
├── gen.image/output/scene/*.png         # Scene images for video generation
├── gen.audio/output/story/*.wav         # Audio chunks (when SOUND_MODE=AUDIO, format: {start_line}_{end_line}.wav)
└── gen.audio/input/audio/               # Audio input directory (created automatically)
```
...
WORKFLOW_SUMMARY_ENABLED = False  # Set to True to enable workflow summary printing
CONCAT_AFTER_COMPLETION = True  # Set to True to merge chunks at the end after all scenes are done, False to merge immediately after each scene
# Dialogue chunks are automatically included in ComfyUI prompts when SOUND_MODE="TEXT"
...
# File Paths
story_file = "../../gen.audio/input/1.story.txt"  # Story file with dialogue format: [speaker] "dialogue"
motion_file = "../input/2.motion.txt"  # Master prompts (optional, generated by 2.motion.py)
...
audio_input_dir = "../../gen.audio/input/audio"  # Audio input directory (created automatically)
final_output_dir = "../output"  # Final video output directory
frames_output_dir = "../output/frames"  # Extracted frames directory
comfyui_input_folder = "../../ComfyUI/input"  # ComfyUI input folder
comfyui_output_folder = "../../ComfyUI/output"  # ComfyUI output folder
...
```mermaid
graph TD
    A[gen.audio/input/1.story.txt] --> B[gen.av/scripts/1.story.py]
...
- **Prompt Enhancement**: Dialogue chunks are automatically added to ComfyUI prompts, talking instructions added for facial/mouth movement

#### AUDIO Mode
- **Dialogue Source**: Audio chunks from `gen.audio/output/story/` (generated by `2.story.py`)
- **Audio Matching**: Each dialogue line maps to audio file `{scene_num}_{scene_num}.wav` (sequential scene order)
...
- **Note**: Dialogue text chunks are NOT added to prompts in AUDIO mode (only audio files are used)
...
```
1. Run gen.audio/scripts/2.story.py → Generates audio chunks in gen.audio/output/story/
   └─> Format: {start_line}_{end_line}.wav (e.g., 14_14.wav for line 14)

2. Run gen.av/scripts/2.av.py with SOUND_MODE="AUDIO"
   ├─> Reads gen.audio/input/1.story.txt to find dialogue lines
   ├─> Matches each dialogue to audio chunk by line number
   ├─> Gets audio duration from file using ffprobe
...
|------|---------|-------|--------|---------|
| 1 | Story Audio Generation | `gen.audio/input/1.story.txt` | `gen.audio/output/story/{line}_{line}.wav` | Run `gen.audio/scripts/2.story.py` to generate audio chunks |
| 2 | Dialogue Line Mapping | `gen.audio/input/1.story.txt` (dialogue line 14) | Audio file: `14_14.wav` | Match dialogue line number to audio chunk filename |
...
#### AV → AV (Internal)
- `gen.audio/input/1.story.txt` → `gen.av/scripts/1.story.py` → `gen.image/input/1.story.txt`
- `gen.audio/input/1.story.txt` → `gen.video/scripts/2.motion.py` (via --output argument) → `gen.av/input/2.motion.txt`
- `gen.audio/input/1.story.txt` + `gen.av/input/2.motion.txt` (optional) → `gen.av/scripts/2.av.py` → `gen.av/output/scene_*.mp4`
...
- **Custom Nodes**:
  - **ComfyUI-OmniVoice-TTS** - Multi-speaker text-to-speech generation (replaces the previous TTS Audio Suite / ChatterBox engine)
...
# Install ComfyUI-OmniVoice-TTS (multi-speaker TTS used by gen.audio/scripts/2.story.py)
...
|--------|-------------|--------------|-------------------|
| `gen.audio/scripts/1.character.py` | `gen.audio/input/1.story.txt`, `gen.audio/voices/` | `gen.audio/input/1.story.speakers.txt` (character→voice-stem map) | LLM-driven voice casting (when `AUTO_ASSIGN_VOICE=True`), chapter summaries |
| `gen.audio/scripts/2.story.py` | `gen.audio/input/1.story.txt`, `gen.audio/input/1.story.speakers.txt` | `gen.audio/output/story.wav` | OmniVoice multi-speaker TTS generation logs |
...
| Script | Input Files | Output Files | Intermediate Files |
|--------|-------------|--------------|-------------------|
| `gen.av/scripts/1.story.py` | `gen.audio/input/1.story.txt` | `gen.image/input/1.story.txt` | Story parsing logs |
| `gen.image/scripts/1.story.py` | `gen.audio/input/1.story.txt` | `gen.image/input/1.story.txt` | Story parsing logs |
...
| `gen.video/scripts/2.motion.py` (shared) | `gen.audio/input/1.story.txt`, `gen.image/output/scene/*.png` (optional), `gen.image/input/2.character.txt` (optional), `gen.image/input/2.location.txt` (optional) | `gen.av/input/2.motion.txt` | Master prompt generation logs |
| `gen.av/scripts/2.av.py` | `gen.audio/input/1.story.txt`, `gen.image/output/scene/*.png`, `gen.av/input/2.motion.txt` (master prompts, optional), `gen.audio/output/story/*.wav` (optional, when SOUND_MODE=AUDIO) | `gen.av/output/scene_*.mp4` (with built-in audio), `gen.av/output/frames/*.png` | AV video generation logs, frames |
...
- `1.story.speakers.txt` - Character→voice-stem map (generated by `1.character.py`, consumed by `2.story.py`)
...
#### Input Files
- **Audio**: `1.story.txt`, `1.story.speakers.txt`, voice samples under `gen.audio/voices/`
...
- **AV**: `gen.audio/input/1.story.txt`, `gen.image/output/scene/*.png`, `gen.av/input/2.motion.txt` (optional), `gen.audio/output/story/*.wav` (optional, when SOUND_MODE=AUDIO)
...
- **AV**: `scene_*.mp4` (with built-in audio), `frames/*.png` (extracted frames), `final.mp4`
